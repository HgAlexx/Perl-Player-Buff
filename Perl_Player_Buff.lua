...
local PPBEC_BuffPerLine = 10; -- coded
local PPBEC_ShowNativeCoolDown = false; -- done
local PPBEC_ShowOriginalTextTimer = true; -- done
local PPBEC_HandleWeaponBuff = false; -- partial, config ok, buff icon todo
local PPBEC_DisplayCastableBuffs = false; -- done
local PPBEC_DisplayCurableDebuff = false; -- done
local PPBEC_ShowSecondUnder10m = false; -- done
...
function Perl_Player_Buff_BuffFrame_UpdateAllBuffAnchors()
    ...
    for buffButtonIndex=1+enchantCount,BUFF_ACTUAL_DISPLAY+enchantCount do
        ...
        -- BETA
        -- deal with cooldown here (case when OnUpdate = nil)
        local cooldownFrame = _G[buff:GetName().."Cooldown"];
        if cooldownFrame then
            CooldownFrame_SetTimer(cooldownFrame, 0, 0, 0);
        end;
...
function Perl_Player_Buff_DebuffButton_UpdateAnchors(buttonName, index)
    ...
    -- BETA
    -- deal with cooldown here (case when AuraButton:OnUpdate = nil)
...
-- copy of the original blizzard function, for planned feature
...
function Perl_Player_Buff_AuraButton_UpdateDuration(auraButton, timeLeft)
    ...
    if (PPBEC_ShowNativeCoolDown and PPBEC_ShowNativeCoolDown == true) then
        ...
        if name == "TempEnchant1" or name == "TempEnchant2" or name == "TempEnchant3" then
            ...
            if auraButton and auraButton.expirationTime and timeLeft and timeLeft > 0 then
                -- if cooldown is already shown, it means is already running ! 
                -- so don't setup it again and again -> cpu saved
                if not cooldownFrame:IsShown() then
                    CooldownFrame_SetTimer(cooldownFrame, auraButton.expirationTime - timeLeft, timeLeft, 1);
                end
            else
                if cooldownFrame:IsShown() then -- same logic as above, is not shown, dont reset it again and again
                    CooldownFrame_SetTimer(cooldownFrame, 0, 0, 0);
